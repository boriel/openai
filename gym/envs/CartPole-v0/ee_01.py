...
class Individual:
    ...
    def __init__(self, tries=10, render=False):
        ...
        self.coef = np.array([random.uniform(0, 1) for _ in range(LENGTH)])
...
if __name__ == '__main__':
    ...
    while I_EP < MAX_EPISODES:
        ...
        if nn_f > best_f:
            best_f = nn_f
            best = nn
            print("Episode:", I_EP, best, best_f)
            cnt = 0

        avg.append(nn_f)
        if len(avg) == MAX_WIN_STEPS:
            if np.array(avg).mean() >= WIN_THRESHOLD:
                break
            avg.pop(0)

    if MONITOR:
        ENV.monitor.close()
    print('Episodes: {}, reward: {}, avg: {}'.format(I_EP, best_f, np.array(avg).mean()))
